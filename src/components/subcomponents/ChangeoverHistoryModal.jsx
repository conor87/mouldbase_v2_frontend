...
export default function ChangeoverHistoryModal({
  isOpen,
  onClose,
  historyId,
  historyRows,
      ...
      <div className="min-h-full flex items-start justify-center py-6">
        <div className="w-full max-w-7xl rounded-2xl bg-slate-800 border border-white/10 shadow-2xl text-white max-h-[90vh] flex flex-col">
          <div className="flex items-center justify-between px-5 py-4 border-b border-white/10 sticky top-0 bg-slate-800 z-10">
            ...
            <button type="button" onClick={onClose} className="px-3 py-1 rounded-lg bg-white/10 hover:bg-white/20" disabled={loading}>
              ✕
            </button>
          </div>
          ...
          <div className="p-5 overflow-y-auto">
            ...
            {!loading && !error && historyRows.length === 0 && <p className="opacity-80">Brak wpisów w historii.</p>}
                  ...
                  <tbody className="align-top">
                    {historyRows.map((h) => (
                      <tr key={String(h.id)} className="border-t border-white/10">
                        ...
                        <td className="px-4 py-3 whitespace-nowrap text-center">{formatDateOnly(h.created)}</td>
                        <td className="px-4 py-3">
                          <pre className="text-xs bg-black/30 p-2 rounded-xl overflow-auto max-h-56">{JSON.stringify(h.old_data ?? null, null, 2)}</pre>
                        </td>
                        <td className="px-4 py-3">
                          <pre className="text-xs bg-black/30 p-2 rounded-xl overflow-auto max-h-56">{JSON.stringify(h.new_data ?? null, null, 2)}</pre>
                        </td>
                      </tr>
                    ))}
                  </tbody>
